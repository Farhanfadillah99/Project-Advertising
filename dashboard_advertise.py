...
engine = create_engine('mysql+mysqlconnector://root:fadillahfarhan99@localhost/advert?host=localhost?port=3306')
conn = engine.connect()
dfAuto=pd.DataFrame(conn.execute('SELECT * FROM advertising_ad').fetchall(),columns=pd.read_csv('advertising_ad.csv').columns)
# dfAuto=pd.read_csv('advertising.csv')
pred_model = pickle.load(open('Advertising_ad_model', 'rb'))[0]
...
app.layout = html.Div(children = [
    ...
    dcc.Tabs(value = 'tabs', id = 'tabs-1', children = [
        dcc.Tab(className = 'col-2',label = 'Table', id = 'table', children = [
        ...
        dcc.Tab(className = 'col-2',label = 'Bar Chart', id = 'bar', children = [
            html.Div(className = 'col-12', children = [
        ...
        dcc.Tab(className = 'col-2',label = 'Scatter Chart', id = 'scatter', children = [
        html.Div(children = dcc.Graph(
            id = 'graph-scatter',
    ...
    dcc.Tab(className = 'col-2',label = 'Pie Chart', id = 'tab-dua', children = [
        html.Div(className = 'col-4',children= [
            html.P('Select value'),
     ...
     dcc.Tab(className = 'col-2',label='Prediction', id='tab-predict', children=[
            html.Div(children=[
                ...
                html.Div(children=[
                    html.Div(children=[
                        ...
                        dcc.Input(id='Age', type='number', value='0', className='col-12',),
                        html.P('Area Income: ', className='ml-2 mr-2'),
                        dcc.Input(id='Area_Income', type='number', value='0', className='col-12',),
                        html.P('Daily Internet Usage: ', className='ml-2 mr-2'),
                        dcc.Input(id='Daily_Internet_Usage', type='number', value='0', className='col-12',),
                        html.P('Male: ', className='col-12'),
                            dcc.Dropdown(
                                id='Male',
                                options=[
                                    {'label': 'yes', 'value': 1},
                       ...
                       html.P('Hour: (1-24)',className='col-12'),
                       dcc.Input(id='Hour',type='number',value='0',className='col-12',),
                        html.Button(children='Predict', id='predict', className='col-12')
    ...
    #  dcc.Tab(className = 'col-2',label='Prediction Image', id='predict-img', children=[
    #         html.Div(children=[
    #             html.H5('Prediction Method', className='ml-2 mr-2'),
    #             html.Div(children=[
    #                 html.Img(src=app.get_asset_url('prediction-image.jpeg'))
    #             ])
    #         ],
    #         ),
    #     ]),
...
def updated_data(n_clicks,size):
    return size
...
@app.callback(
    ...
    State(component_id='Hour', component_property='value')]
)
def predict_legendary(n_clicks, Daily_Time_Spent_on_Site, Age, Area_Income, Daily_Internet_Usage, Male, Hour):
    if n_clicks is None:
        raise dash.exceptions.PreventUpdate

    if Daily_Time_Spent_on_Site == '0' or Age == '0' or Area_Income == '0' or Daily_Internet_Usage == '0' or Male == '' or Hour == '0':
        children = 'Please fill all needed value!'
    else:
        X_test = np.array([Daily_Time_Spent_on_Site, Age, Area_Income, Daily_Internet_Usage, Male, Hour]).reshape(1, -1)
