...
pred_model = pickle.load(open('Advertising_model', 'rb'))
...
def predict_legendary(n_clicks, Daily_Time_Spent_on_Site, Age, Area_Income, Daily_Internet_Usage, Male):
    ...
    if Daily_Time_Spent_on_Site == '0' or Age == '0' or Area_Income == '0' or Daily_Internet_Usage == '0' or Male == '':
        ...
        X_test = np.array([Daily_Time_Spent_on_Site, Age, Area_Income, Daily_Internet_Usage, Male]).reshape(1, -1)[0][0]
        predict_result = pred_model.predict(X_test)
        pds = log_loss(y_test,model.predict_proba(X_test))
